...
2. Open the file `CarRentalExampleG1.sql` located in the root of this project folder.
3. Click **Execute** (or press `F5`) to create the `CarRentalExampleG1` database and all tables.

### 2. Update the Connection String

Open `CarRentalExampleG1/appsettings.json` and update the `Server` value to match your SQL Server instance name:

```json
"ConnectionStrings": {
    "ConnString": "Server=YOUR_SERVER_NAME;Database=CarRentalExampleG1;Trusted_Connection=true;TrustServerCertificate=true;"
}
```

> Replace `YOUR_SERVER_NAME` with your actual server name (e.g. `localhost`, `.\SQLEXPRESS`, or `DESKTOP-ABC123\SQLEXPRESS`).

### 3. Run the Project

Open `CarRentalExampleG1.slnx` in Visual Studio and press **F5**, or run from the terminal:

```bash
dotnet run --project CarRentalExampleG1
```

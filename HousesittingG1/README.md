...
2. Open `HousesittingG1.sql` from the root of this project folder.
3. Click **Execute** (or press `F5`) to create the `HousesittingG1` database and all tables.

### 2. Update the Connection String

Open `HousesittingG1/appsettings.json` and update the `Server` value to match your SQL Server instance name:

```json
"ConnectionStrings": {
    "conn": "Server=YOUR_SERVER_NAME;Database=HousesittingG1;Trusted_Connection=True;TrustServerCertificate=True;"
}
```
...
Open `HousesittingG1.slnx` in Visual Studio and press **F5**, or run from the terminal:

```bash
dotnet run --project HousesittingG1
```

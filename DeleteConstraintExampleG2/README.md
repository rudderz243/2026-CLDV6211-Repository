# Delete Constraint Example — Group 2
...
2. Open `StudentG2.sql` from the root of this project folder.
3. Click **Execute** (or press `F5`) to create the `StudentG2` database and all tables.

### 2. Update the Connection String

Open `DeleteConstraintExampleG2/appsettings.json` and update the `Server` value to match your SQL Server instance name:

```json
"ConnectionStrings": {
    "conn": "Server=YOUR_SERVER_NAME;Database=StudentG2;Trusted_Connection=True;TrustServerCertificate=True;"
}
```
...
Open `DeleteConstraintExampleG2.slnx` in Visual Studio and press **F5**, or run from the terminal:

```bash
dotnet run --project DeleteConstraintExampleG2
```

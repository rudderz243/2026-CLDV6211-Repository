...
2. Open `BirdWatcherG1.sql` from the root of this project folder.
3. Click **Execute** (or press `F5`) to create the `BirdWatcherG1` database and the `Spottings` table.

### 2. Update the Connection String

Open `BirdWatcherG1/appsettings.json` and update both connection strings:

```json
"ConnectionStrings": {
    "birdy": "Server=YOUR_SERVER_NAME;Database=BirdWatcherG1;Trusted_Connection=True;TrustServerCertificate=True;",
...
Open `BirdWatcherG1.slnx` in Visual Studio and press **F5**, or run from the terminal:

```bash
dotnet run --project BirdWatcherG1
```
